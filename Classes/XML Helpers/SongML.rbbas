...
Module SongML
	...
	#tag Method, Flags = &h1
		Protected Function Draw_Footer(songDoc As XmlDocument, g As Graphics, x As Integer, y As Integer, width As Integer, zoom As Double) As Integer
		  ...
		  If Len(ext) > 0 Then
		    ext = "Copyright " + encodings.UTF8.Chr(169) + " " + ext  'Gotta use the UTF8 code point to avoid a spurious character on display for some reason
		    height = height + GraphicsX.DrawFontString(g, ext, x, y-height, tempFont, width, "center", 0, "bottom")
		  End If
		...
		Protected Function Draw_Header(songDoc As XmlDocument, g As Graphics, x As Integer, y As Integer, width As Integer, zoom As Double, Page As Group2D) As Integer
		  ...
		  If titleWidth + authorWidth + songnoWidth >  width Then  //Put author on a new line
		    ...
		    If songnoWidth > 0 Then
		      SongNumHeight = DrawFontString(g, title, x, y, songnoFont, width, Header, "right")
		    End If
	...
	#tag Method, Flags = &h21
		Private Function Draw_SoloChordLine(songDoc As XmlDocument, g As Graphics, x As Integer, y As Integer, ColWidth As Integer, zoom As Double, ByRef chords() As String, ByRef Page As Group2D) As Integer
		  ...
		  For i = 1 To UBound(chords)
		    ...
		    ChordFont.OntoGraphics g
		    chordHeight = g.TextHeight
		    chordWidth = g.StringWidth(chords(i) + "M") ' Don't get the chords too close together, add a little gap
		    s = New StringShape
		    s.x = x + (chordWidth / 2)
		    s.y = y + g.TextAscent
		    s.Text = chords(i)
		    ...
		    If SmartML.GetValueB(SmartML.GetNode(songDoc.DocumentElement, "capo"), "@print") Then
		      ...
		      capoChordWidth = g.StringWidth(capoChord + "M") 'Don't get the chords too close together, add a little gap
		      s = New StringShape
		      ChordFont.OntoStringShape s
		      s.Text = capoChord
	...
	#tag Method, Flags = &h21
		Private Function FullHeading(heading As String, fullVerses As Boolean = False) As String
		  ...
		  ElseIf Uppercase(Left(heading, 1)) = "C" Then   ' -- CHORUS --
		    If heading.Len > 1 Then
		      heading = " Chorus " + Mid(heading, 2) + " "
		    Else
		      heading = " Chorus "
		    End If
		  ElseIf Uppercase(Left(heading, 1)) = "B" Then   ' -- BRIDGE --
		    If heading.Len > 1 Then
		      heading = " Bridge " + Mid(heading, 2) + " "
		    Else
		      heading = " Bridge "
		    End If
		  ElseIf Uppercase(Left(heading, 1)) = "T" Then   ' -- TAG --
		    If heading.Len > 1 Then
		      heading = " Tag " + Mid(heading, 2) + " "
		    Else
		      heading = " Tag "
		    End If
		  End If
	...
	#tag Method, Flags = &h0
		Function ToHTML(songElement As XmlNode) As String
		  ...
		  s = s + "  <div id=""title"">" + SmartML.GetValue(songElement, "title") + "</div>" + EndOfLine
		  s = s + "  <div id=""author"">" + SmartML.GetValue(songElement, "author") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "time_sig").Len > 0 Then s = s + "  <div id=""time_sig"">" + App.T.Translate("advanced_song_editor/time_sig/@caption") + ": " + SmartML.GetValue(songElement, "time_sig") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "tempo").Len > 0 Then s = s + "  <div id=""tempo"">" + App.T.Translate("advanced_song_editor/tempo/@caption") + ": " + SmartML.GetValue(songElement, "tempo") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "capo").Len > 0 Then s = s + "  <div id=""capo"">" + App.T.Translate("advanced_song_editor/capo/@caption") + " " + SmartML.GetValue(songElement, "capo") + "</div>" + EndOfLine
		  ...
		  For l = 1 To UBound(lines)
		    If Left(lines(l), 1) = "." And l < UBound(lines) And InStr("123456789 ", Left(lines(l+1), 1)) > 0 Then
		      ...
		      For i = 0 To sliceCount - 1 ' Loop through each chord slice
		        If i = 0 Then
		          s = s + "      <td class=""chords"">" + Trim(Mid(slices(i*lineCount+1),2)) + "&nbsp;</td>" + EndOfLine
		        Else
		          s = s + "      <td class=""chords"">" + Trim(slices(i*lineCount+1)) + "&nbsp;</td>" + EndOfLine
		        End If
		      Next i
		      s = s + "    </tr>" + EndOfLine
		      ...
		      For j = 2 To lineCount ' Loop through the lines and print
		        s = s + "    <tr>" + EndOfLine
		        For i = 0 To sliceCount - 1 ' Loop through each slice
		          If slices(i*lineCount+j).Len = 0 Then slices(i*lineCount+j) = "&nbsp;"
		    ...
		    ElseIf Left(lines(l), 1) = "." Then
		      s = s + "  <div class=""chords"">" + Mid(lines(l), 2) + "</div>" + EndOfLine
		      
		    ElseIf Left(lines(l), 1) = ";" Then
		      s = s + "  <div class=""comment"">" + Mid(lines(l), 2) + "</div>" + EndOfLine
		      
		    ElseIf Left(lines(l), 1) = "[" Then
		      s = s + "  <p/><div class=""heading"">" + FullHeading(Mid(lines(l), 2, lines(l).Len-2), True) + "</div>" + EndOfLine
		      
		    Else
		      s = s + "  <div class=""lyrics"">" + Mid(lines(l), 2, lines(l).Len-1) + "</div>" + EndOfLine
		      
		    End If
		  Next l
		  s = s + "<br/>" + EndOfLine
		  
		  If SmartML.GetValue(songElement, "aka").Len > 0 Then s = s + "  <div id=""aka"">" + App.T.Translate("advanced_song_editor/aka/@caption") + " " + SmartML.GetValue(songElement, "aka") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "copyright").Len > 0 Then s = s + "  <div id=""copyright"">" + App.T.Translate("general_song_editor/copyright/@caption") + " " + SmartML.GetValue(songElement, "copyright") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "ccli").Len > 0 Then s = s + "  <div id=""ccli"">" + App.T.Translate("general_song_editor/ccli/@caption") + " " + SmartML.GetValue(songElement, "ccli") + "</div>" + EndOfLine
		  If ColonTrim(SmartML.GetValue(songElement, "theme")).Len > 0 Then s = s + "  <div id=""themes"">" + App.T.Translate("advanced_song_editor/themes/@caption") + ": " + ColonTrim(SmartML.GetValue(songElement, "theme")) + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "key_line").Len > 0 Then s = s + "  <div id=""key_line"">" + App.T.Translate("advanced_song_editor/key_line/@caption") + ": " + SmartML.GetValue(songElement, "key_line") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "key").Len > 0 Then s = s + "  <div id=""key"">" + App.T.Translate("advanced_song_editor/key/@caption") + " " + SmartML.GetValue(songElement, "key") + "</div>" + EndOfLine
		  
		  If SmartML.GetValue(songElement, "user1").Len > 0 Then s = s + "  <div id=""user1"">" + MainWindow.lbl_song_user1_1.Text + " " + SmartML.GetValue(songElement, "user1") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "user2").Len > 0 Then s = s + "  <div id=""user2"">" + MainWindow.lbl_song_user2.Text + " " + SmartML.GetValue(songElement, "user2") + "</div>" + EndOfLine
		  If SmartML.GetValue(songElement, "user3").Len > 0 Then s = s + "  <div id=""user3"">" + MainWindow.lbl_song_user3.Text + " " + SmartML.GetValue(songElement, "user3") + "</div>" + EndOfLine
		  
		  s = s + "<p><a class=""opensong"" href=""http://www.opensong.org/"">" + App.T.Translate("about/created_with_opensong") + "</a></p>" + EndOfLine
		  s = s + "</body></html>"
	...
	#tag Method, Flags = &h0
		Sub ToSetML(songElement As XmlNode, style As XmlNode = Nil)
		  ...
		  '--
		  If Len(SubtitleOptions) = 0 Then
		    SubtitleOptions = SmartML.GetValue(App.MyPresentSettings.DocumentElement, "style/@song_subtitles")
		  End If
		  Subtitles = Split(SubtitleOptions, "|")
		  Separator = App.T.Translate("songml/separator/@caption")
		  If Separator = "|NL|" Then Separator = Chr(10)
		  
		  For CurrSubtitle = 0 To UBound(Subtitles)
		    Select Case Subtitles(CurrSubtitle)
		      
		    Case "copyright"
		      
		      If Len(SmartML.GetValue(songElement, "copyright")) > 0 Then
		        If Len(SubtitleText)  > 0 Then SubtitleText = SubtitleText + Separator
		        '++JRC 12/05
		        ...
		        End If
		        SubtitleText = SubtitleText + SmartML.GetValue(songElement, "copyright")
		      End If
		      
		    Case "hymn_number"
		      
		      If Len(SmartML.GetValue(songElement, "hymn_number")) > 0Then
		        If Len(SubtitleText)  > 0 Then SubtitleText = SubtitleText + Separator
		        '++JRC 12/05
		        'Made the extra text optional
		        if SmartML.GetValueB(App.MyPresentSettings.DocumentElement, "style/@descriptive_subtitle_info") = true then
		          SubtitleText = SubtitleText + App.T.Translate("songml/hymn_number/@caption") + " " + _
		          SmartML.GetValue(songElement, "hymn_number")
		        else
		          SubtitleText = SubtitleText + SmartML.GetValue(songElement, "hymn_number")
		        end if
		        '--
		      End If
		    ...
		    Case "author"
		      ...
		      If Len(SmartML.GetValue(songElement, "author")) > 0 Then
		        If Len(SubtitleText)  > 0 Then SubtitleText = SubtitleText + Separator
		        '++JRC 12/05
		        'Made the extra text optional
		        if SmartML.GetValueB(App.MyPresentSettings.DocumentElement, "style/@descriptive_subtitle_info") = true then
		          SubtitleText = SubtitleText + App.T.Translate("songml/author/@caption") + " " + _
		          SmartML.GetValue(songElement, "author")
		        else
		          SubtitleText = SubtitleText + SmartML.GetValue(songElement, "author")
		        end if
		        '--
		      End If
		    ...
		    Case "ccli_number"
		      ...
		      If Len(SmartML.GetValue(songElement, "ccli")) > 0 Then
		        If Len(SubtitleText)  > 0 Then SubtitleText = SubtitleText + Separator
		        '++JRC 12/05
		        'Made the extra text optional
		        if SmartML.GetValueB(App.MyPresentSettings.DocumentElement, "style/@descriptive_subtitle_info") = true then
		          SubtitleText = SubtitleText + App.T.Translate("songml/ccli_number/@caption") + " " + _
		          SmartML.GetValue(songElement, "ccli")
		        else
		          SubtitleText = SubtitleText + SmartML.GetValue(songElement, "ccli")
		        end if
		        '--
		      End If
		    ...
		    Case "ccli_license"
		      ...
		      If Len(SmartML.GetValue(App.MyMainSettings.DocumentElement, "ccli/@number")) > 0 Then
		        If Len(SubtitleText)  > 0 Then SubtitleText = SubtitleText + Separator
		        '++JRC 12/05
		        'Made the extra text optional
		        if SmartML.GetValueB(App.MyPresentSettings.DocumentElement, "style/@descriptive_subtitle_info") = true then
		          SubtitleText = SubtitleText + App.T.Translate("songml/ccli_license/@caption") + " " + _
		          SmartML.GetValue(App.MyMainSettings.DocumentElement, "ccli/@number")
		        else
		          SubtitleText = SubtitleText + SmartML.GetValue(App.MyMainSettings.DocumentElement, "ccli/@number")
		        end if
		        '--
		      End If
