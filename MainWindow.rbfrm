		  ...
		  While slide_group <> Nil
		    If SmartML.GetValue(slide_group, "@type", True) = "song" Then
		      ...
		      If songDoc <> Nil Then
		        ...
		        Log.CCLISongNumber = SmartML.GetValue(SongDoc.DocumentElement, "ccli_number", True)  //The song's CCLI numbe
...
#tag Events btn_songs_export
	#tag Event
		Sub Action()
		  ...
		  Dim expFolder, indexHtmlFile As FolderItem
		  Dim styleCSSFile As FolderItem = App.AppFolder.Child("OpenSong Settings").Child("style.css")
		  Dim songFileOutput, indexHTMLoutput As TextOutputStream
		  ...
		  App.DoEvents
		  indexHtmlFile = expFolder.Child("index.html")
		  indexHTMLoutput = indexHtmlFile.CreateTextFile
		  If Nil = indexHTMLoutput Then Exit Sub
		  
		  indexHTMLoutput.WriteLine "<html><head>"
		  indexHTMLoutput.WriteLine "<meta http-equiv=""Content-type"" content=""text/html;charset=UTF-8"" />"
		  indexHTMLoutput.WriteLine "<title>" + App.T.Translate("songs_mode/songs/@caption") + "</title><link rel=""stylesheet"" href=""style.css"" type=""text/css""/></head><body>"
		  indexHTMLoutput.WriteLine "<div id=""title"">" + App.T.Translate("songs_mode/songs/@caption") + "</div><ul id=""song_list"">"
		  
		  App.MouseCursor = WatchCursor
		  ProgressWindow.Show
		  
		  For i = 0 To lst_songs_songs.ListCount - 1
		    Dim foldername As String = lst_songs_songs.CellTag(i, 0).StringValue
		    
		    // Create folders if necessary
		    Dim currentFolderPart As String = MakeSafeURLName(foldername, False)
		    Dim separator As Integer
		    Dim workingFolder As FolderItem = expFolder
		    
		    separator = InStr(separator, currentFolderPart, "/")
		    while separator > 1
		      If Not workingFolder.Child(Left(currentFolderPart, separator - 1)).Exists Then
		        workingFolder.Child(Left(currentFolderPart, separator - 1)).CreateAsFolder
		      End If
		      
		      If Not workingFolder.Child(Left(currentFolderPart, separator - 1)).Directory Then
		        // a file is blocking the way for our folder
		        workingFolder.Child(Left(currentFolderPart, separator - 1)).Delete
		        workingFolder.Child(Left(currentFolderPart, separator - 1)).CreateAsFolder
		      End If
		      
		      // go one deeper
		      workingFolder = workingFolder.Child(Left(currentFolderPart, separator - 1))
		      currentFolderPart = Right(foldername, Len(foldername) - separator)
		      separator = InStr(separator, currentFolderPart, "/")
		      
		      // copy .css if needed
		      If styleCSSFile <> Nil And Not workingFolder.Child(styleCSSFile.Name).Exists Then styleCSSFile.CopyFileTo workingFolder
		    wend
		    
		    songFileOutput = workingFolder.Child(MakeSafeURLName(lst_songs_songs.List(i), False) + ".html").CreateTextFile
		    song = SmartML.XDocFromFile(Songs.GetFile(foldername + "/" + lst_songs_songs.List(i)))
		    If songFileOutput <> Nil And song <> Nil Then
		      //songFileOutput.Write SongML.ToHTML(song.DocumentElement)
		      If indexHTMLoutput <> Nil Then indexHTMLoutput.WriteLine "<li><a href=""" + MakeSafeURLName(foldername, True) + MakeSafeURLName(lst_songs_songs.List(i), True) + ".html"">" + lst_songs_songs.List(i) + "</a></li>"
		      songFileOutput.Close
		    End If
		  ...
		  Next i
		  If indexHTMLoutput <> Nil Then
		    indexHTMLoutput.WriteLine "</ul><p><a class=""opensong"" href=""http://www.opensong.org/"">" + App.T.Translate("about/created_with_opensong") + "</a></p>"
		    indexHTMLoutput.Write "</body></html>"
		  End If
		  indexHTMLoutput.Close
		  App.MouseCursor = Nil
		  styleCSSFile = App.AppFolder.Child("OpenSong Settings").Child("style.css")
		  If styleCSSFile <> Nil Then styleCSSFile.CopyFileTo expFolder
