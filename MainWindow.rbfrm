   ...
   Begin PagePanel pge_contents
      ...
      Begin Canvas can_editor_external_slide
         ...
         Begin SEditField edt_external_notes
            ...
            BehaviorIndex   =   156
         End
         Begin SButton btn_external_powerpoint
            AcceptFocus     =   "True"
            AcceptTabs      =   "False"
            AutoDeactivate  =   "True"
            Backdrop        =   0
            ...
            BehaviorIndex   =   157
         End
         Begin SButton btn_external_impress
            AcceptFocus     =   "True"
            AcceptTabs      =   "False"
            AutoDeactivate  =   "True"
            Backdrop        =   0
	...
	#tag Method, Flags = &h0
		Sub ActionInSetSave()
		  ...
		  Case "image"
		    ...
		    lst_set_items.List(CurrentInSetItem) = edt_image_name.Text + " (" + SmartML.GetValue(xgroup, "@type", True) + ")"
		  Case "style"
		  ...
		  case "external"
		    ...
		    SmartML.SetValueB xgroup, "@looppresentation", chk_external_loop_presentation.Value
		    
		    if rad_external_host_impress.Value then
		      SmartML.SetValue xgroup, "@host", "impress"     'TH: change to constant
		      SmartML.SetValue xgroup, "@filename", edt_external_presentation_file.Text
		      SmartML.SetValueB xgroup, "@looppresentation", chk_external_loop_presentation.Value
		      
		    end if
		    if rad_external_host_powerpoint.Value then
		      SmartML.SetValue xgroup, "@host", "ppt"     'TH: change to constant
		      SmartML.SetValue xgroup, "@filename", edt_external_presentation_file.Text
		      SmartML.SetValueB xgroup, "@looppresentation", chk_external_loop_presentation.Value
		      
		    end if
		    if rad_external_host_pptview.Value then
		      SmartML.SetValue xgroup, "@host", "pptview"     'TH: change to constant
		      SmartML.SetValue xgroup, "@filename", edt_external_presentation_file.Text
		      SmartML.SetValueB xgroup, "@looppresentation", chk_external_loop_presentation.Value
		    end if
	...
	#tag Event
		Sub Change()
		  ...
		  Case "image"
		    ...
		    chk_image_descriptions.Value = chk_image_loop.Value = SmartML.GetValueB(xgroup, "@descriptions_in_subtitle", False)
		  ...
		  Case "external"
		    ...
		    dim host as string 
		    host = SmartML.GetValue( xgroup , "@host" )
		    
		    select case host
		    case "ppt"  ' TH : change to constant
		      btn_external_powerpoint.SetStuck(true)
		      btn_external_impress.SetStuck(false)
		      ...
		      chk_external_loop_presentation.Value = SmartML.GetValueB( xgroup , "@looppresentation" )
		      
		      
		    case "impress"  ' TH : change to constant
		      btn_external_powerpoint.SetStuck(true)
		      btn_external_impress.SetStuck(false)
		      btn_external_videolan.SetStuck(false)
		      btn_external_application.SetStuck(false)
		    ...
		    case "pptview"  ' TH : change to constant
		      btn_external_powerpoint.SetStuck(true)
		      btn_external_impress.SetStuck(false)
		      btn_external_videolan.SetStuck(false)
		      btn_external_application.SetStuck(false)
...
#tag Events edt_external_notes
	...
	#tag EndEvent
#tag EndEvents
#tag Events btn_external_powerpoint
	#tag Event
		Sub MouseExit()
		  SetHelp ""
		End Sub
	...
	#tag Event
		Sub Action()
		  btn_external_powerpoint.SetStuck( Not btn_external_powerpoint.GetStuck() )
		  btn_external_impress.SetStuck( False )
		  btn_external_videolan.SetStuck( False )
		  btn_external_application.SetStuck( False )
		  
		  If btn_external_powerpoint.GetStuck() Then
		    '+TH
		    'All host choices are available to powerpoint
		    
		    rad_external_host_pptview.Enabled=true
		    rad_external_host_powerpoint.Enabled=true
		    rad_external_host_impress.Enabled=true
		    'default is powerpoint, probably should try and detect whether this is actually possible!
		    rad_external_host_powerpoint.Value=true
	...
	#tag EndEvent
#tag EndEvents
#tag Events btn_external_impress
	#tag Event
		Sub MouseExit()
		  SetHelp ""
		End Sub
	...
	#tag Event
		Sub Open()
		  'Me.SetIcon impress_pic, impress_mask
		End Sub
	#tag EndEvent
	#tag Event
		Sub Action()
		  btn_external_powerpoint.SetStuck( False )
		  btn_external_impress.SetStuck( Not btn_external_impress.GetStuck() )
		  btn_external_videolan.SetStuck( False )
		  btn_external_application.SetStuck( False )
		  
		  If btn_external_impress.GetStuck() Then
		    '+TH
		    'we can only show open office presenations using open office
		    rad_external_host_impress.Value=true
		    'prevent other choices
		    rad_external_host_pptview.Enabled=false
		    rad_external_host_powerpoint.Enabled=false
		    rad_external_host_impress.Enabled=true
	...
	#tag Event
		Sub MouseEnter()
		  SetHelp "external_slide/impress"
...
#tag Events btn_external_application
	...
	#tag Event
		Sub Open()
		  'Me.SetIcon application_pic, application_mask
		End Sub
	#tag EndEvent
	#tag Event
		Sub Action()
...
#tag Events btn_external_presentation_file
	...
	#tag Event
		Sub Action()
		  ...
		  dim  f as new OpenDialog()
		  f.InitialDirectory=SpecialFolder.Documents
		  
		  dim fi as FolderItem
		  f.Filter=PresentationFileTypes.All
		  fi=f.ShowModal()
		  
		  if (fi <> nil) then
		    edt_external_presentation_file.Text = f.Result.AbsolutePath
		  end if
