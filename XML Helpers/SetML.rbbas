...
Module SetML
	...
	#tag Method, Flags = &h1
		Protected Sub DrawSlide(g As Graphics, xslide As XmlNode, Style as SlideStyle)
		  ...
		  aspect_ratio = gWidth / gHeight 'EMP 09/05
		  
		  'If xstyle <> Nil Then background = SmartML.GetValueP(xstyle, "background", True)
		  ...
		  If background <> Nil Then
		    'strip = (g.Height/background.Height) * SmartML.GetValueN(xstyle, "background/@strip_footer")
		    strip = (g.Height/background.Height) * Style.StripFooter 'EMP 9/05
		    //++EMP 09/05, 12/05
		    ...
		    Case SlideStyle.POS_STRETCH
		      ...
		      // Else taken out -- background color already painted
		      'Else
		      ''If SmartML.GetValueC(xstyle, "background/@color", c) Then g.ForeColor = c
		      'g.ForeColor = Style.BGColor 'EMP 9/05
		  ...
		  'While g.StringWidth(line) / (g.Width - (2*RealBorder)) * GraphicsX.FontFaceHeight(g, bodyStyle) > MainHeight * .85 ' last number offsets the non-perfectness of this guessing
		  ...
		  For i = 1 To UBound(lines)
		    If (g.StringWidth(lines(i)) > UsableWidth * 2) Or (multiwrap And g.StringWidth(lines(i)) > UsableWidth) Then
		      ...
		      'While y <= Len(line) And g.StringWidth(Mid(line, x, y-x)) < g.Width - (2*RealBorder)
		      ...
		      For z = y DownTo x
		        ...
		        End If
		      Next z
		      'ElseIf g.StringWidth(lines(i)) > g.Width - (2*RealBorder) Then ' this line is less than twice as long, but still too long: smart wrap it
		  ...
		  'DrawFontString g, line, RealBorder, HeaderSize, bodyStyle, g.Width - RealBorder*2, Style.BodyAlign, MainHeight, Style.BodyVAlign
